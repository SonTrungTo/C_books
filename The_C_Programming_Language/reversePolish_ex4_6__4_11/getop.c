...
static char    line[MAXLINE];             /* an array containing 1 input line */
...
int getop(char s[]) {
  ...
  if (buf == 0)                             /* These avoid using ungetch using internal stactic variable, buf */
    c = getch();
  else {
    c   = buf;
    buf =   0;
  }
  while ((s[0] = c) == ' ' || c == '\t')    /* Dumbing garbage */
    c = getch();
  s[1] = '\0';
  i = 0;
  if (islower(c)) {
    ...
    s[i] = '\0';
    if (c != EOF)
      buf = c;                                    /* Went one character too far */
